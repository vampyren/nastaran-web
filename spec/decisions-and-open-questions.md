...
| Foreground listener default cadence: **~10 min** via `ScheduleWakeup` while session is open; owner can force an immediate check by command ("check the queue now" / "pick it up" / "process the queue"). Closing the session ends the listener — no persistence. | Queue pickup needn't be near-instant and dev work takes time anyway, so ~10-min idle polling cuts token spend. A ~60 s default was tried and judged wasteful; faster pickup is available on demand. |
