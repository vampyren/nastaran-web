...
| Safe edit surface = `src/content/{berattelser,home,kontakt,om-mig,site}.ts`, **plus a narrow content-driven renderer glue allowance** (minimal same-page `src/app/<page>/page.tsx` wiring only to display a new content field). | Matches Nastaran's existing content layer. Anything else = unsafe → `failed + manualFix`. Glue rule added 2026-05-28 (see entry below). |
...
| Foreground listener default cadence: **~10 min** via `ScheduleWakeup` while session is open; owner can force an immediate check by command ("check the queue now" / "pick it up" / "process the queue"). Closing the session ends the listener — no persistence. A faster **quiet ~60 s review-state decision-watch** is allowed while a pushed request awaits an owner decision (see entry below). | Queue pickup needn't be near-instant and dev work takes time anyway, so ~10-min idle polling cuts token spend. A ~60 s *idle* default was tried and judged wasteful; faster pickup is on demand, with ~60 s reserved for the review-state decision-watch. |
...
**Content-driven renderer glue allowed (2026-05-28):** the safe edit surface stays `src/content/*.ts`, but the operator may now also make **minimal same-page renderer wiring** (a new content field in `src/content/<page>.ts` + display-only changes to the matching `src/app/<page>/page.tsx`) **without per-request owner approval**, when a content request cannot render through the content file alone. Only inside the `req/<id>-<slug>` branch + PR + preview flow; gates + preview + Publicera still required before production. **Not** auto-allowed: route/API/auth/config changes, admin-pipeline internals, broad layout redesign, global styling/design-system rewrites, unrelated shared-component refactors, or cross-page edits — those still stop-and-ask or `failed + manualFix`. Reason: today's `/kontakt` "Vad händer sen?" request needed `src/content/kontakt.ts` + a minimal section in `src/app/kontakt/page.tsx` because the renderer is hardcoded per-section; forcing every such request through manual approval is unnecessary friction. Captured in CLAUDE.md rule 8, `pipeline-operator-modes.md` § Four-tier classification rule, `pipeline-mvp.md` § Safe edit surface, and `docs/REUSABLE-REQUEST-QUEUE-PATTERN.md`.

**Review-state decision-watch cadence added (2026-05-28):** Mode A has no push signal from the website/API — Claude Code only notices owner button actions (Publicera / Förbättra / Avvisa) on a wake/check, and we deliberately add **no** daemon / webhook / cron / child `claude -p` / background worker. To shorten the wait, while a request the operator pushed sits at `review` the listener may poll on a faster **quiet ~60 s** cadence until that request becomes `done` / `improve_requested` / `rejected` / `failed`, then returns to the ~10 min idle cadence. Decision-watch ticks stay quiet unless state changes or an error fires. Reason: today's Publicera was first picked up on a later ~10 min wake — correct but slow. Owner-facing note (decision picked up automatically within ~1 min while under review; manual immediate check still available) lives in `docs/PIPELINE-HANDOFF.md`. Captured in CLAUDE.md rule 5 and `pipeline-operator-modes.md` § Foreground listener.
