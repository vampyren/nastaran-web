...
7. **Single-lane.** At most one active request across `in_progress | review | improve_requested | publishing` (the `LANE_BLOCKING_STATUSES` set in `src/lib/request-types.ts`). `improve_requested` reuses **same request, same branch, same PR** — never duplicates a PR. **`clarification_needed` is NOT a lane occupant** — it's parked waiting for the requester's answer (no branch/PR) and never blocks other `queued` requests; the operator skips it when scanning. It counts only toward the intake queue-depth cap (`QUEUE_DEPTH_STATUSES`), which is a *counting* set, not a lane-busy check. Don't conflate the two.
   - **Ambiguous-but-safe → `clarification_needed` (not stop-and-ask).** When a content request is ambiguous but likely safe (e.g. page/text conflict, or "tweak the intro" with no specifics), the operator CAS-writes `queued → clarification_needed` with a concise Swedish `clarificationQuestion` (no claim, no branch, no PR) and **keeps the listener running**. The requester answers in the queue board (**Svara** → `POST /api/clarify/[id]`), which flips it back to `queued`; the operator re-picks up the same request and reads the Q+A. Never guess page/wording/placement/image-usage. Structural/unsafe still → `failed + manualFix`; genuine owner-policy decisions can still stop and ask the owner. Full detail in `spec/pipeline-operator-modes.md` § Four-tier classification rule § Clarification flow.
