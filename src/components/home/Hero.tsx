...
/**
 * Hero layout:
 * - SectionMarker + hairline span the full section width above everything.
 * - Below xl: stacked. Copy first, image panel below.
 * - xl+ (≥ 1280px): 2-column grid — content + image panel.
 */
    ...
    >
      <div className={SECTION_WIDTH}>
        <SectionMarker
          ...
          className="mb-3"
        />
        <div aria-hidden className={`mb-6 ${HAIRLINE_GRADIENT}`} />
        <div className="xl:grid xl:grid-cols-[minmax(0,1fr)_minmax(300px,380px)] xl:gap-x-[46px] xl:items-stretch">
          {/* Hero copy */}
          <div>
            <Reveal>
              <p className={`mb-4 ${EYEBROW} text-accent-deep`}>
                {hero.prelude}
              </p>
              <h1
                id={hero.headingId}
          ...
          {/* Ritual panel — stacked below copy until xl, then column 2 */}
          <Reveal
            delay={0.12}
            className="mt-8 xl:mt-0 xl:self-stretch"
          >
            <aside
