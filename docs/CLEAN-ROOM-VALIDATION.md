...
13. **Clarification flow.** Submit an ambiguous-but-safe request (e.g. text says "första sidan" but the page picker says Berättelser). The queue worker (CC) does **not** guess — it CAS-writes `queued → clarification_needed` with a Swedish `clarificationQuestion`, **no branch/PR**. The card appears under **Väntar på svar** with the original request text, attachments (if any), and the question. **Lane check:** while it waits, submit a second clear request and confirm CC does **NOT** start it — `clarification_needed` holds the single lane until resolved. Click **Svara**, answer → request flips back to `queued` (`clarificationAnswer` + history `clarification_answered` written), still the **same** request id, still no branch/PR. CC re-picks it up, reads Q+A, processes to `review`; the previously-waiting second request can then be picked up. Verify: no duplicate request/branch/PR; **Avvisa** from `clarification_needed` → `rejected`; clarify answer when not in `clarification_needed` → 409, missing id → 404, bad id/input → 400, no cookie → 401, bad origin → 403.
